...
if [ -n "$(echo $relversion| sed -n "/^[0-9]\+$/p")" ] && [[ $relversion < 7 ]] ;then
   yum -y install tix libX11 db4 tcl tk wget git gcc MySQL-python epel-release mysql-server mysql-devel
elif [ -n "$(echo $relversion| sed -n "/^[0-9]\+$/p")" ] && [[ $relversion > 6 ]] ;then
   yum -y install tix libX11 db4 tcl tk wget git gcc MySQL-python epel-release mariadb-server mariadb-devel
else
